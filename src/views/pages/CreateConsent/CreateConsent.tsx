  ...
  const handleCreateConsentSubmit = (event: any) => {
    ...
    if (form.checkValidity() === true) {
      auth.user.jwt().then(jwt => {
        createConsentRequest(form.elements.enterNumber.value, jwt).then(response => {
          const consent_handle = response.data["consent_handle"];
          if (consent_handle)
            globalThis.window.location.href =
              `https://anumati.setu.co/${consent_handle}?redirect_url=${getPathToDashboard()}`;
              setValidated(false);
              setShowError(false);
        }).catch(() => {
          setShowError(true);
  ...
  return (
    <Container>
      ...
      {!showLoader &&
        <>
        <Col sm='12'>
          <CreateConsentForm validated={validated} handleCreateConsentSubmit={handleCreateConsentSubmit} />
        </Col>
        <Col sm='12'>
          {!showError &&
            <CreateConsentImage height='500px' imgHeight='250px' width='100%' />
          }
          {!!showError &&
